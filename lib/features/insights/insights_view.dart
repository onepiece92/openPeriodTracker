...
class InsightsView extends StatelessWidget {
  ...
  bool _hasPhaseData(
    Map<CyclePhase, Map<String, int>> moods,
    Map<CyclePhase, Map<String, int>> symptoms,
  ) {
    return moods.values.any((m) => m.isNotEmpty) ||
        symptoms.values.any((s) => s.isNotEmpty);
  }
      ...
      child: Column(
        ...
        children: [
          ...
          if (inWindow)
            ...
            _intelRow(
              '📅',
              'Fertile window opens in $daysToFertile day${daysToFertile == 1 ? '' : 's'}. Sperm can survive 5 days — plan accordingly.',
            )
          else
            _intelRow(
              '🌙',
              'Fertile window has passed for this cycle. Next window estimated in ${avg - cycleDay + fertileStart} days.',
            ),
        ],
      ),
    );
    ...
    return Row(
      ...
      children: [
        ...
        Expanded(
          child: Text(
            text,
            style: AppTextStyles.body.copyWith(fontSize: 11, height: 1.4),
          ),
        ),
      ],
    );
...
class _WeeklyScore extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final color = weekPct >= 70
        ? AppColors.ovulation
        : weekPct >= 40
        ? const Color(0xFFD4A340)
        : AppColors.menstrual;
    final label = weekPct >= 70
        ? 'Great'
        : weekPct >= 40
        ? 'Fair'
        : 'Low';
    ...
    return Container(
      ...
      child: Column(
        children: CyclePhase.values.map((phase) {
          final moods =
              (phaseMoods[phase]!.entries.toList()
                    ..sort((a, b) => b.value.compareTo(a.value)))
                  .take(3);
          final symptoms =
              (phaseSymptoms[phase]!.entries.toList()
                    ..sort((a, b) => b.value.compareTo(a.value)))
                  .take(3);
                ...
                Expanded(
                  child: Wrap(
                    ...
                    children: [
                      ...moods.map(
                        (e) => _chip(
                          '${_moodEmoji[e.key] ?? ''}${e.key}',
                          AppColors.phaseColor(phase),
                        ),
                      ),
                      ...symptoms.map(
                        (e) => _chip(e.key, const Color(0xFF5BA4A4)),
                      ),
                    ],
                  ),
                ),
...
class _SymptomForecast extends StatelessWidget {
  ...
  const _SymptomForecast({
    required this.cycleDaySymptoms,
  ...
  Widget build(BuildContext context) {
    ...
    for (int d = currentDay; d < currentDay + 5 && d <= avgCycle; d++) {
      ...
      if (syms != null && syms.isNotEmpty) {
        final top =
            (syms.entries.toList()..sort((a, b) => b.value.compareTo(a.value)))
                .first;
        forecasts.add(
          _ForecastItem(day: d, symptom: top.key, count: top.value),
        );
      }
    }
...
class _PersonalizedTips extends StatelessWidget {
  ...
  List<_Tip> _generateTips() {
    ...
    final topSymptom = mySymptoms.isNotEmpty
        ? (mySymptoms.entries.toList()
                ..sort((a, b) => b.value.compareTo(a.value)))
              .first
              .key
        : null;
    final topMood = myMoods.isNotEmpty
        ? (myMoods.entries.toList()..sort((a, b) => b.value.compareTo(a.value)))
              .first
              .key
        : null;
    ...
    switch (phase) {
      case CyclePhase.menstrual:
        return const _Tip(
          ...
          body:
              'Rest is productive right now. Your body is doing important work. Honor the slowdown — it fuels the energy burst coming in your follicular phase.',
        );
      case CyclePhase.follicular:
        return const _Tip(
          emoji: '🌱',
          ...
          body:
              'Your brain is sharpest for learning and planning. Start that project, schedule important meetings, try something new. Estrogen is your superpower right now.',
        );
      case CyclePhase.ovulation:
        return const _Tip(
          emoji: '⭐',
          ...
          body:
              'Peak communication and confidence. Great for presentations, difficult conversations, and social events. Your verbal fluency is literally higher right now.',
        );
      case CyclePhase.luteal:
        return const _Tip(
          emoji: '🌙',
          title: 'Luteal Phase',
          body:
              'Detail-oriented tasks suit this phase. Finish projects, organize, nest. Cravings are your body asking for serotonin — complex carbs deliver it naturally.',
        );
    }
  }

  _Tip _symptomTip(String symptom, CyclePhase phase) {
    final phaseLabel = AppColors.phaseName(
      phase,
    ).replaceAll(' Phase', '').toLowerCase();
    switch (symptom) {
      case 'Cramps':
        return _Tip(
          emoji: '🫙',
          title: 'Your cramps pattern',
          body:
              'You tend to get cramps during your $phaseLabel phase. Pre-load with magnesium 2-3 days before. A heating pad at 104°F for 20 min is as effective as ibuprofen.',
        );
      case 'Headache':
        return _Tip(
          emoji: '💧',
          title: 'Headache alert',
          body:
              'Headaches are common for you in the $phaseLabel phase. Start hydrating extra today — dehydration + hormone shifts are the #1 combo trigger.',
        );
      case 'Fatigue':
        return _Tip(
          emoji: '🔋',
          ...
          body:
              'Fatigue peaks for you during $phaseLabel. Schedule lighter workloads if possible. Iron-rich snacks (pumpkin seeds, dark chocolate) help more than caffeine.',
        );
      case 'Bloating':
        return _Tip(
          emoji: '🫧',
          title: 'Bloating season',
          body:
              'Your $phaseLabel bloating pattern suggests fluid retention. Cut sodium today, add potassium (banana, avocado), and try warm lemon water in the morning.',
        );
      default:
        return _Tip(
          emoji: '📋',
          title: 'Track it',
          body:
              '$symptom is your most common symptom in the $phaseLabel phase. Consistent logging helps identify what triggers it and what relieves it.',
        );
    }
  }
    ...
    switch (mood) {
      ...
      case 'Sleepy':
        return const _Tip(
          ...
          title: 'Rest is data',
          body:
              'You frequently feel tired in this phase. That\'s not laziness — it\'s progesterone. Go to bed 30 min earlier tonight. Your body will thank you tomorrow.',
        );
      case 'Anxious':
      case 'Stressed':
        return const _Tip(
          ...
          body:
              'Anxiety tends to spike for you right now. Try box breathing (4-4-4-4) for 2 minutes. Magnesium glycinate before bed helps. Reduce caffeine by 50% this phase.',
        );
      case 'Sad':
      case 'Lonely':
        return const _Tip(
          ...
          body:
              'Lower mood is common in this phase for you. It\'s hormonal, not personal. Reach out to someone you trust. Omega-3 and sunlight are natural mood lifters.',
        );
      case 'Happy':
      case 'Energetic':
      case 'Confident':
        return const _Tip(
          ...
          title: 'Ride the wave',
          body:
              'You feel great in this phase! Use this energy wisely — schedule important decisions, social events, and challenging workouts now.',
        );
      default:
        return _Tip(
          emoji: '🧠',
          title: 'Mood insight',
          body:
              'Your dominant mood in this phase is $mood. Tracking this pattern helps you plan your week around your cycle\'s natural rhythm.',
        );
    }
  }
    ...
    switch (phase) {
      case CyclePhase.menstrual:
        return const _Tip(
          ...
          title: 'Movement',
          body:
              'Gentle walks, yin yoga, or light stretching. Even 10 minutes improves cramp pain by increasing blood flow. Don\'t push — listen to your body.',
        );
      case CyclePhase.follicular:
        return const _Tip(
          emoji: '🏃‍♀️',
          title: 'Movement',
          body:
              'Energy is climbing — try HIIT, strength training, or a new fitness class. Your muscles recover faster and you build strength more efficiently in this phase.',
        );
      case CyclePhase.ovulation:
        return const _Tip(
          emoji: '💪',
          title: 'Movement',
          body:
              'Peak performance window. Go for PRs, group classes, or competitive sports. Your pain tolerance is highest and reaction times are fastest.',
        );
      case CyclePhase.luteal:
        return const _Tip(
          emoji: '🧘‍♀️',
          title: 'Movement',
          body:
              'Moderate intensity — Pilates, swimming, steady cycling. High cortisol + intense exercise = worse PMS. Gentle consistency beats hard sessions this week.',
        );
    }
  }
}
